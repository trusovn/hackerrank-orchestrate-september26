# Foundation Readiness Review

## Verdict
READY / READY_WITH_NOTES / BLOCKED

## Command verification

| Capability | Command | Result | Notes |
|---|---|---|---|
| Bootstrap/install | | PASS/FAIL/N/A | |
| Build/typecheck | | | |
| Focused/smoke test | | | |
| Full/normal verify | | | |
| Lint/static | | | |
| Run/boot | | | |
| Migration smoke | | | |
| AI fake/fixture test | | | |
| AI eval smoke | | | |

## Agent-legibility check
- Project purpose discoverable:
- Canonical commands discoverable:
- Placement rules discoverable:
- Architecture/current constraints discoverable:
- Generated vs editable files clear:
- Failure diagnostics available:

## AI foundation check
N/A or:
- Provider seam:
- Deterministic fake/fixture:
- Output validation:
- Retry/idempotency:
- Trace/debug path:
- Eval seam:

## Blockers
1. <evidence / impact / smallest repair / re-verification>

## Notes
- <non-blocking issue only if useful>

## Recommended delivery route
`PERSONAL_FLOW | SDD_QUICK | SDD_STANDARD | SDD_FULL`

### Reasons
- <specific project evidence>

### Next step
<exact next skill/stage>
