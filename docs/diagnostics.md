...
| `LOGIC-DETERMINISTIC` | A pure rule returns the wrong value or violates an invariant; focused unit assertion shows input and expected behavior. | Use the package [`navigation map`](../code/buy_or_wait/README.md) to select the owning module, public starting point, and mirrored focused test. | Minimal regression passes, then the owning and affected downstream suites pass. | Available for evidence resolution, lifecycle/FX, forecasting, safety replay, baseline capacity, and no-change candidate generation; later policy/output stages remain deferred. |
