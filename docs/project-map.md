...
| [`../code/evaluation/`](../code/evaluation/) | Evaluation runner and usage-report source packaged under `evaluation/`; `evidence_strategy.py` implements the WP-03 offline evidence-strategy gate and `main.py` implements the WP-09B public-sample oracle and baseline metrics. | `python3 code/evaluation/main.py` (offline, deterministic; prints a 25-row public-sample JSON report without writing by default). |
...
| [`wp-09-plan.md`](wp-09-plan.md) | WP-08C-acceptance-gated, sequential WP-09A–F contracts for one-case composition, isolated public oracle/metrics, the five bounded experiments, conservative selection, and one frozen global policy. | WP-09A accepted (2026-09-13). WP-09B implemented and ready for review: `code/evaluation/main.py` isolates solved sample labels, compares all 25 predictions, and prints deterministic metrics; run `python3 -m unittest tests.test_evaluation` or `python3 code/evaluation/main.py`. Next: fresh independent WP-09B acceptance, then WP-09C. |
...
| Public-sample evaluation behavior | [`../code/evaluation/main.py`](../code/evaluation/main.py) and [`../tests/test_evaluation.py`](../tests/test_evaluation.py) | `python3 -m unittest tests.test_evaluation` |
| Usage accounting | [`../code/evaluation/usage_report.md`](../code/evaluation/usage_report.md); usage report remains a placeholder | Feature-owned usage command must be added with implementation |
