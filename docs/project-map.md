...
| [`wp-07-plan.md`](wp-07-plan.md) | Sequential implementation contracts for exact no-change candidates, recurring-series spending changes, changed-candidate replay, deterministic ranking, and status/method derivation. | Implement WP-07A, WP-07B, and WP-07C sequentially with fresh acceptance between parts; each part has a 12–15 call soft checkpoint and all share `planning.py`/`test_planning.py`. |
