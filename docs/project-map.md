...
| [`../code/main.py`](../code/main.py) | Thin batch CLI/composition entry point; product behavior is not implemented. | `python3 code/main.py`. |
 | [`../code/buy_or_wait/`](../code/buy_or_wait/) | Repository-owned product Python: provider-neutral AI boundary, immutable domain contract, strict dataset repository, and deterministic typed-evidence resolver. Before broad source inspection, read [`../code/buy_or_wait/README.md`](../code/buy_or_wait/README.md) and start from the listed owning symbols/tests. | `buy_or_wait.ai_boundary`, `buy_or_wait.domain`, `buy_or_wait.repository`, `buy_or_wait.evidence`. |
...
| [`wp-04-plan.md`](wp-04-plan.md) | Accepted-gate implementation contract for lifecycle normalization, pending reserves, source accounting, and exact directed settlement-date FX. | WP-02 settlement-date correction is accepted (2026-09-12); run the short prerequisite gate on the accepted bytes, then implement independently of in-progress optional WP-03. |
| [`wp-05-plan.md`](wp-05-plan.md) | WP-04-acceptance-gated implementation contract for strict recurrence, finite variable/income/date policies, exclusive source ownership, and a replayable baseline ledger. | Begin only after accepted WP-04 preserves canonical source money and exposes exact conversion for synthesized settlement dates. |
